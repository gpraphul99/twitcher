...
$response['status']=true;
    ...
    if(!$form_data['last_name']){
        $response['msg']="Enter Last name";
        $response['status']=false;
        $response['field']='last_name';
    }
    if(!$form_data['first_name']){
        $response['msg']="Enter First name";
        $response['status']=false;
...
function validateLoginForm($form_data){
    ...
    $blank=false;
        ...
        if(!$form_data['username_email']){
            $response['msg']="Enter Email/Username!";
            $response['status']=false;
...
function createUser($data){
 ...
 $query = "INSERT INTO users(first_name,last_name,gender,email,username, ac_status, password) ";
 $query.="VALUES ('$first_name','$last_name',$gender,'$email','$username','$ac_status', '$password')"; 
 return mysqli_query($db,$query);
}
...
function validateUpdateForm($form_data,$image_data){
    ...
    $response['status']=true;
       ...
       if($image_data['name']){
        ...
        if($size>1000){
            $response['msg']="Please, upload image less then 1 mb";
            $response['status']=false;
            $response['field']='profile_pic';
        }
       }
